            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title" style="padding-top: 10px;">OR Management List</h2>
                        <button class="btn btn-outline-success float-right" data-toggle="modal" data-target="#AddModal">
                            <span class="fas fa-plus"></span> Create OR
                        </button>
                    </div>
                    <div class="card-body">
                        <table id="example3" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                @foreach ($or_management as $or)
                                    <tr>
                                        <td><input type="checkbox" /></td>
                                        @foreach ($branches as $branch)
                                            @if($or->branch_id == $branch->id)
                                                @php 
                                        ...
                                        @foreach ($programs as $program)
                                            ...
                                            @endif
                                        @endforeach
                                        <td id="{{ $or->id; }}_branch_code">{{ $branch_code; }}</td>
                                        <td id="{{ $or->id; }}_branch_city">{{ $branch_city; }}</td>
                                        <td id="{{ $or->id; }}_branch_branch">{{ $branch_branch; }}</td>
                                        <td id="{{ $or->id; }}_program_code">{{ $program_code; }}</td>
                                        <td id="{{ $or->id; }}_program_desc">{{ $program_desc; }}</td>
                                        <td id="{{ $or->id; }}_or_code">{{ $or->or_code; }}</td>
                                        <td id="{{ $or->id; }}_created_at">{{ $branch->created_at; }}</td>
                                        <td id="{{ $or->id; }}_updated_at">{{ $branch->updated_at; }}</td>
                                        <td>
